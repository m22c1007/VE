...
public class OVRPlayerController : MonoBehaviour
{
	...
	{
		Transform parent = CameraRig.trackingSpace;

		if (hit.gameObject.tag == "slope")
		{
			Debug.Log("Con:" + Controller.transform.position);
			Debug.Log("Eye:" + CameraRig.centerEyeAnchor.transform.position);
			Debug.Log(parent.position);
			Debug.Log(parent.rotation);



			if (!(parent.transform.rotation == hit.transform.rotation))
			{
				Debug.Log("test");
